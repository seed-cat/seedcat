...
rm -f "$NAME.zip"
zip -r "$NAME.zip" . -i $HC/hashcat.exe -i $HC/hashcat.bin -i $HC/hashcat.hcstat2 $HC/modules/*.so \
  $HC/modules/*.dll $HC/OpenCL/*.cl $HC/OpenCL/*.h $HC/charsets/* $HC/charsets/*/*
zip -ju "$NAME.zip" "./target/x86_64-pc-windows-gnu/release/$NAME.exe"
zip -ju "$NAME.zip" "./target/x86_64-unknown-linux-gnu/release/$NAME"